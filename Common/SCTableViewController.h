...
@property(atomic,          ) NSInteger pageNo;
// tableView每一行对应的Model class， 必须是SCTableViewCellData的子类
@property(nonatomic, strong) NSArray<Class>* cellDataClassArray;
...
@property(nonatomic,strong) NSArray* footerHeightArray;
@property(nonatomic,      ) CGRect   customFrame;

// 你必须设置以下两个中的一个：
// 如果你用xib定义每一个cell，请提供xib名字（不包含.xib后缀）
@property(nonatomic, strong) NSArray<NSString*>* cellNibNameArray;
// 否则，提供一个SCTableViewCell的子类
@property(nonatomic, strong) NSArray<Class>* cellClassArray;
@property(nonatomic, strong) NSArray<Class>* headerClassArray;
@property(nonatomic, strong) NSArray<Class>* footerClassArray;
@property(nonatomic, strong) NSArray<NSString*>* headerTitleArray;
@property(nonatomic, strong) NSArray<NSString*>* footerTitleArray;
